...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        // number of categories (aka length of menuVar and categoryVar).
        int numberOfCategories = StdIn.readInt();

        // Initalize menuVar and categoryVar
        menuVar = new MenuNode[numberOfCategories];
        categoryVar = new String[numberOfCategories];

        for (int j = 0; j < numberOfCategories; j++) {

            String nameOfCategory = StdIn.readString();
            categoryVar[j] = nameOfCategory;
            ...
            for (int i = 0; i < numberOfDishes; i++) {
                ...
                int numberOfIngredientIds = StdIn.readInt();

                int[] storeIds = new int[numberOfIngredientIds];

                for (int c = 0; c < numberOfIngredientIds; c++) {
                    int storeCurrentId = StdIn.readInt();
                    storeIds[c] = storeCurrentId;
                }

                Dish currentDish = new Dish(nameOfCategory, nameOfDish, storeIds);

                MenuNode currentMenuNode = new MenuNode(currentDish, null);

                menuVar[j] = insertToFront(currentMenuNode, j, menuVar);
    ...
    // Returns the new front of MenuNode LL
    private static MenuNode insertToFront(MenuNode tobeAdded, int j, MenuNode[] menuVar) {
        MenuNode currentFront = menuVar[j];

        if (currentFront == null) {
            return tobeAdded;
        }

        tobeAdded.setNextMenuNode(currentFront);
        return tobeAdded;
    }
    ...
    public void addStockNode(StockNode newNode) {
        ...
        // index of where to be inserted
        int hashFunction = ingredientID % stockVarSize;

        stockVar[hashFunction] = insertStockToFront(newNode, hashFunction, stockVar);

    }

    private StockNode insertStockToFront(StockNode newNode, int index, StockNode[] insert) {
        StockNode front = null;
        if (insert[index] == null) {
            insert[index] = newNode;
            front = newNode;
            return front;
        }

        newNode.setNextStockNode(insert[index]);
        front = newNode;
        return front;
    }
        ...
        for (int i = 0; i < stockVar.length; i++) {
            ...
            while (current != null) {
                ...
                if (current.getIngredient().getName().equals(ingredientName)) {
                    // stockVar[i] is the head of the LL
                    if (current == stockVar[i]) {
    ...
    public StockNode findStockNode(int ingredientID) {
        ...
        StockNode front = stockVar[hashFunction];
        StockNode current = front;
    ...
    public void updateStock(String ingredientName, int ingredientID, int stockAmountToAdd) {
        ...
        // WRITE YOUR CODE HERE
        StockNode tobeFound = null;

        if (ingredientName == null) {
            tobeFound = findStockNode(ingredientID);
            tobeFound.getIngredient().updateStockLevel(stockAmountToAdd);
            return;
        } else if (ingredientID == -1) {
            tobeFound = findStockNode(ingredientName);
            tobeFound.getIngredient().updateStockLevel(stockAmountToAdd);
            return;
    ...
    public void updatePriceAndProfit() {
        ...
        for (MenuNode category : menuVar) {
            ...
            while (ptr != null) {
                double cost = 0;
                int[] stockIdForDish = ptr.getDish().getStockID();

                for (int ID : stockIdForDish) {
                    StockNode current = findStockNode(ID);
                    if (current == null)
                        continue;
                    cost += current.getIngredient().getCost();
                }
                ...
                // stock ID array --> int id --> ingredient item
    ...
    public void createStockHashTable(String inputFile) {
        ...
        while (StdIn.hasNextLine()) {
            ...
            Ingredient current = new Ingredient(ID, ingredientName, stockAmount, cost);
            StockNode tobeAdded = new StockNode(current, null);
            addStockNode(tobeAdded);
        }
        // WRITE YOUR CODE HERE
    }
    ...
    public boolean checkDishAvailability(String dishName, int numberOfDishes) {
        ...
        // WRITE YOUR CODE HERE
        MenuNode ofDish = findDish(dishName);
        if (ofDish == null)
            return false;

        Dish found = ofDish.getDish();

        int[] stockID = found.getStockID();

        for (int id : stockID) {
            StockNode current = findStockNode(id);
            if (current == null)
                return false;
            if (current.getIngredient().getStockLevel() < numberOfDishes)
                return false;
        }
    ...
    public void order(String dishName, int quantity) {
        ...
        if (checkDishAvailability(dishName, quantity) == true) {
            MenuNode found = findDish(dishName);
            Dish customerOrder = found.getDish();
            TransactionData order = new TransactionData("order", dishName, quantity,
                    customerOrder.getProfit() * quantity, true);
            addTransactionNode(order);
            int[] arr = customerOrder.getStockID();
            for (int id : arr) {
                updateStock(null, id, quantity * -1);
            }
            return;
        } else if (checkDishAvailability(dishName, quantity) == false) {
            TransactionData order2 = new TransactionData("order", dishName, quantity, 0, false);
            addTransactionNode(order2);
            MenuNode tobeSearched = findDish(dishName);
            MenuNode ptr = tobeSearched.getNextMenuNode();
            if (ptr == null) {
                ptr = menuVar[findCategoryIndex(tobeSearched.getDish().getCategory())];
            }
            String c = ptr.getDish().getCategory();

            while (ptr != tobeSearched) {
                // Check if the dish is avaliable
                if (checkDishAvailability(ptr.getDish().getDishName(), quantity) == true) {
                    Dish customerSim = ptr.getDish();
                    TransactionData simOrder = new TransactionData("order", ptr.getDish().getDishName(), quantity,
                            customerSim.getProfit() * quantity, true);
                    addTransactionNode(simOrder);
                    int[] arr = customerSim.getStockID();
                    for (int id : arr) {
                        updateStock(null, id, quantity * -1);
                    }
                    return;
                } else if (checkDishAvailability(ptr.getDish().getDishName(), quantity) == false) {

                    TransactionData failOrder = new TransactionData("order", ptr.getDish().getDishName(), quantity,
                            0, false);
                    addTransactionNode(failOrder);
                }

                ptr = ptr.getNextMenuNode();

                // If we reach end of list go back to the front
                if (ptr == null) {
                    // find front
                    ptr = menuVar[findCategoryIndex(c)];
                }
    ...
    public void donation(String ingredientName, int quantity) {
        ...
        StockNode c = findStockNode(ingredientName);

        double checkProfit = profit();

        if (checkProfit > 50 && c.getIngredient().getStockLevel() >= quantity) {
            TransactionData succDonation = new TransactionData("donation", ingredientName, quantity, 0, true);
            addTransactionNode(succDonation);
            updateStock(ingredientName, -1, quantity * -1);
            return;
        } else {
            TransactionData failDonation = new TransactionData("donation", ingredientName, quantity, 0, false);
            addTransactionNode(failDonation);
            return;
        }
    ...
    public void restock(String ingredientName, int quantity) {
        ...
        StockNode c = findStockNode(ingredientName);
        double cost = c.getIngredient().getCost() * quantity;

        if (totalProfit >= cost) {
            updateStock(ingredientName, -1, quantity);
            TransactionData successfulReStock = new TransactionData("restock", ingredientName, quantity, cost * -1.0,
                    true);
            addTransactionNode(successfulReStock);
            return;
        }

        TransactionData unsucData = new TransactionData("restock", ingredientName, quantity, 0, false);
        addTransactionNode(unsucData);
        return;
    }
    ...
    public void seatAllGuests(Queue<People> waitingQueue) {
        ...
        while (!waitingQueue.isEmpty()) {
            ...
            for (int j = 0; j < tablesInfo[0].length; j++) {
                if (tablesInfo[1][j] == 0) {
                    if (tablesInfo[0][j] >= waitingQueue.peek().getNumberInParty()) {
                        tables[j] = waitingQueue.dequeue();
                        seated = true;
                    }
                }
            }
